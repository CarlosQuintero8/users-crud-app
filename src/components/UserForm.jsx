...
export const UserForm = ({ postUser, editUser, userToUpdate, handleCloseModal, isLoading }) => {
	...
	const handleUserSubmit = async (data) => {
		if (file) {
			...
			const { hasError, urlImage, errorMessage } = await imageUpload(file);
			if (hasError) {
				console.log('Hubo un error al subir la imagen:', errorMessage);
				toastError(errorMessage || 'Error al subir la imagen. El usuario se creará sin imagen.');
				setImageIsUploading(false);
				// Generar avatar por defecto con avatar.iran.liara.run usando solo first_name
				const username = encodeURIComponent(data.first_name.trim());
				const gender = data.gender === 'female' ? 'girl' : 'boy'; // Determinar género
				data.image_url = `https://avatar.iran.liara.run/public/${gender}?username=${username}`;
			} else {
		...
		// Si no hay archivo ni imagen editada, generar un avatar por defecto
		if (!file && !imageEdit) {
			const username = encodeURIComponent(data.first_name.trim());
			const gender = data.gender === 'female' ? 'girl' : 'boy'; // Determinar género
			data.image_url = `https://avatar.iran.liara.run/public/${gender}?username=${username}`;
		...
		if (userToUpdate) {
			await editUser('/users', userToUpdate.id, transformedData);
			handleCloseModal();
			return;
		}

		await postUser('/users', transformedData);
		handleCloseModal();
	};

	return (
		<form className="form" onSubmit={handleSubmit(handleUserSubmit)}>
			<div className="form__image">
				<div className="form__image-content">
					<figure onClick={() => fileInputRef.current.click()} className="form__figure">
						<img src={fileDataURL || imageEdit || noUserImage} alt="Foto de usuario" className="form__img" />
					</figure>
				...
				<ButtonPrimarySoft onClick={() => fileInputRef.current.click()} disabled={isLoading || imageIsUploading}>
					Seleccionar
				</ButtonPrimarySoft>
				<input
					type="file"
			...
			<div className="form__field">
				...
				/>
				{errors.first_name && <span className="form__error">{errors.first_name.message}</span>}
			</div>
			<div className="form__field">
			...
			<div className="form__field">
				...
				/>
				{errors.birthday && <span className="form__error">{errors.birthday.message}</span>}
			</div>
			<div className="form__button">
				<ButtonPrimary type="submit" disabled={isLoading || imageIsUploading}>
					{imageIsUploading ? (
						<>
